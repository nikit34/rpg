...
func (ui *ui) CheckDroppedItem() *game.Item {
	...
	if invRect.HasIntersection(&sdl.Rect{
		int32(mousePos.X),
		int32(mousePos.Y),
		1,
		1,
...
func (ui *ui) DrawInventory(level *game.Level) {
	...
	ui.renderer.Copy(
		ui.textureAtlas,
		&playerSrcRect,
		&sdl.Rect{invRect.X + invRect.X / 4, invRect.Y + offset, invRect.W / 2, invRect.H /2},
	)
...
func (ui *ui) CheckGroundItems(level *game.Level) *game.Item {
	if !ui.currentMouseState.leftButton && ui.prevMouseState.leftButton {
		...
		for i, item := range items {
			...
			intersection := itemRect.HasIntersection(&sdl.Rect{int32(mousePos.X), int32(mousePos.Y), 1, 1})
			if intersection {
				return item
			}
		}
